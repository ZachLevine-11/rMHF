...
We can conclude that time trends for met-minutes do not differ significantly between 2020 and 2019.
...
All the autocorrelations for the residual series are non-significant, which is what we'd like. The model passes the Box-Ljung test as well. In other words, our model appears to work. But because our model is a white noise model, it is not forcastable by definition because its values at different times are statistically insignificant. In addition, the fact that both our autoregressive and moving average components have an estimated order of 0 suggests that maybe we don't need an ARIMA model here after all.

Is interaction with dummy intervention variable significant?
