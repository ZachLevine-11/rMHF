...
The largest statistically significant lag values of the correlogram gives the possible *q* values for the ARIMA model. In addition, the largest statistically significant lag of the partial correlogram gives the *p* value for an ARIMA model. Looking back at our ACF and PACFs above, we note that our time series appears to fit best a *white noise* model.
...
There are no autoregressive or moving average terms required here. We're left with a white noise model, telling us that observations at each time point are statistically independent of oneanother.
...
Note that this is a white-noise model. Checking our assumptions.
...
All the autocorrelations for the residual series are non-significant, which is what we'd like. The model passes the Box-Ljung test as well. In other words, our model appears to work. Because our model is a white noise model, then it is not forcastable by definition because its values at different times are statistically insignificant.


Alright, so we got the same point predicted every time - the mean of the ARIMA model. However, the fact that both our autoregressive and moving average components have an estimated order of 0 suggests that maybe we don't need an ARIMA model here after all. Let's try fitting a regression line and grabbing those predictions as well. We should also check the diagnostic plots for our 2020 model.
...
post2020 <- df[18:22,]
#Fix the column names of both objects so we can fit the regression models.
colnames(pre2020) <- c(colnames(pre2020)[1:10], "mets")
colnames(post2020) <- c(colnames(post2020)[1:10], "mets")
#Properly type the met-minutes column of each dataset.
pre2020$mets <- as.numeric(pre2020$mets)
post2020$mets <- as.numeric(post2020$mets)
#Add a week number column to both datasets to regress on time.
pre2020$weekNum <- 1:5
post2020$weekNum <- 7:11
#Fit the regression model.
lm2020_before <- lm(pre2020, formula =  mets ~ weekNum)
plot(lm2020_before)
anova(lm2020_before)
```

Alright, so our model appear to work, though the interaction with time is nonsignificant. It's time to forecast some values after the intervention.

```{r}
pred <- predict(lm2020_before)
```

Let's grab the differences between the predictions generated by our model and the actual values:

```{r}
diff <- pred[1:5] - post2020$mets
print(diff)
```

Plotting these differences against their index, we obtain the following:

```{r fig.width = 5}
library(ggplot2)
plotDf <-  data.frame("differences" = diff, "indices" = 1:5, stringsAsFactors = FALSE)
ggplot(data = plotDf, mapping = aes(indices, differences)) + geom_point() + labs(x = "Index", y = "Difference between fitted and true value", title = "Differences")
```

Let's also plot the differences against the true values.
```{r fig.width = 5}
plotDf <-  data.frame("true" = post2020$mets,
                      "pred" = pred[1:5],
                      "indices" = 1:5,
                      stringsAsFactors = FALSE)
ggplot(data = plotDf,
       mapping = aes(indices)) +
  geom_line(aes(y = true, colour = "darkblue")) +
  geom_line(aes(y = pred, colour = "red")) +
  labs(x = "Index",
      y = "MET-minutes",
      title = "Differences") +
  scale_color_discrete(name = "type", labels = c("actual values", "fitted values"))
```


##Volume of visits

Automatic ARIMA model fitting yields the following.

```{r}
pre2020$"% of patients who were no-shows" <- as.numeric(pre2020$"% of patients who were no-shows")
forecast::auto.arima(pre2020$"% of patients who were no-shows")
```

With no autoregressive or moving average components, it's not clear that we need an ARIMA model, at all. Let's try simple linear regression.
