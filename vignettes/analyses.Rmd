...
Auto arima agrees with this result. But this wasn't our main focus. It's time for some intervention analysis on the 2020 data. If we fit an ARIMA model to the 2020 [Jan 1 - March11] data automatically, we yield the following.
...
```{r}
tseries::kpss.test(ts(RMHF::read_data()[11:16, "Met-minutes"]))
manualfit <- forecast::Arima(ts(RMHF::read_data()[11:16, "Met-minutes"]), order = c(1,1,0))
```
...
forecast::checkresiduals(manualfit)
manualfit
```
