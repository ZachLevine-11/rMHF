...
This tells us the ideal ARIMA model is a white noise model. Let's then fit another ARIMA model incuding a regressor that codes for the intervention.

```{r}
intervention <- c(rep(0, 6), rep(1, 5))
fit_include_intervention <- arima(RMHF::read_data()[12:22, "Met-minutes"], order = c(0,0,0), xreg = intervention)
```
...
```{r}
lmtest::lrtest(fit_include_intervention, autofit)
```
...
```{r}
intervention <- c(rep(0, 6), rep(1, 5))
fit_include_intervention <- arima(RMHF::read_data()[12:22, "Prescheduled appointments"], order = c(0,0,0), xreg = intervention)
```
...
```{r}
intervention <- c(rep(0, 6), rep(1, 5))
fit_include_intervention <- arima(RMHF::read_data()[12:22, "% of patients who were no-shows"], order = c(0,0,0), xreg = intervention)
```
...
We want to see whether the intervention had a significant impact on the relationship between attendance and met-minute interactions.

What we're going to do here is include an interaction term between intervention and time, whose coefficient (trend) we want to compare. Since we think that the effect of attendance on met-minutes in 2020 might have a different effect before and after 2020, we include an interaction term between attendance and the intervention. Then we'll fit  a regression model to the data.

The coefficient for % attendance is the coefficient for weeks before the intervention only. The interaction term between year and weeks represents the difference in the coefficients between before and after the intervention. This will give us a p value if we test the statistical significiance of the interaction effects which what we'd like. 

```{r fig.height = 5, fig.width = 5}
mets2020 <- RMHF::read_data()[12:22, "Met-minutes"]
attendance2020 <- as.numeric(RMHF::read_data()[12:22,]$"% of patients who were no-shows")
intervention <- c(rep(0, 6), rep(1, 5))
fitdf <- data.frame(mets2020, attendance2020, intervention, stringsAsFactors = FALSE)
colnames(fitdf) <- c("mets", "attendance", "intervention")
fit <- lm(data = fitdf, formula = mets ~ attendance  + attendance*intervention)
plot(fit)
summary(fit)
```
