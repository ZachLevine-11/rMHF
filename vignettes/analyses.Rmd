...
post2020 <- df[18:22,]
#Fix the column names of both objects so we can fit the regression models.
colnames(pre2019) <- c(colnames(pre2019)[1:10], "mets")
colnames(post2019) <- c(colnames(post2019)[1:10], "mets")
colnames(pre2020) <- c(colnames(pre2020)[1:10], "mets")
colnames(post2020) <- c(colnames(post2020)[1:10], "mets")
#Properly type the met-minutes column of each dataset.
pre2019$mets <- as.numeric(pre2019$mets)
post2019$mets <- as.numeric(post2019$mets)
pre2020$mets <- as.numeric(pre2020$mets)
post2020$mets <- as.numeric(post2020$mets)
#Add a week number column to both datasets to regress on time.
pre2019$weekNum <- 1:5
post2019$weekNum <- 7:11
pre2020$weekNum <- 1:5
...
Lets check some diagnostic plots for our 2020 model.
...
Alright, so our model appear to work. It's time to forecast some values after the intervention.
...
```{r fig.width = 5, fig.height = 5}
library(ggplot2)
plotDf <-  data.frame("differences" = abs(diff), "indices" = 1:5, stringsAsFactors = FALSE)
