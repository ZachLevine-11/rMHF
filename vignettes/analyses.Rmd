...
First, let's use two tests of stationarity (Augmented Dickey-Fuller, KPSS) to determine whether the time series is stationary.

**ADF testing**
Let's start with the Augmented-Dickey-Fuller test. Here, the null hypothesis is that the series is non-stationary, that is, the existence of a unit root. The alternative hypothesis is that the series is stationary.
...
Alright, but what about the KPSS test? Let's give it a try on our original MET-minute data. Recaall that the KPSS test is a different hypothesis test than the ADF one used above. Here, the null is that the data is stationary. For this test, we'd like not to reject H0, so we'd like the p value to be greater than 0.05.
...
Alright, so both p values were less than our significance level, so we have to reject H0 and conclude that the 
data is nonstationary. Let's try first differencing again, similar to the above.

```{r}
# Differencing 2019 data.
tseries::kpss.test(tsdf2019mets_diff1)

# Differencing 2020 data.
tseries::kpss.test(tsdf2020mets_diff1)
```

Second differencing now.

```{r}
# Second differencing 2019 data.
tseries::kpss.test(tsdf2019mets_diff2)

# Second differencing 2020 data.
tseries::kpss.test(tsdf2020mets_diff2)
```

Third differencing now.

```{r}
# Third differencing 2019 data.
tseries::kpss.test(tsdf2019mets_diff3)

# Third differencing 2020 data.
tseries::kpss.test(tsdf2020mets_diff3)
```

Trying to make 2020 data stationary.

```{r}
# Testing fourth differenced 2020 data.
tseries::kpss.test(diff(tsdf2020mets, differences = 4))
# Testing fifth differenced 2020 data.
tseries::kpss.test(diff(tsdf2020mets, differences = 5))

```

**Synthesizing the ADF and KPSS tests**

The KPSS test showed a p value for 2019 *D = 3* very close to the edge of the rejection zone. This agrees with the ADF test, so we can conclude that it is reasonable. The KPSS test doesn't indicate stationarity for the 2020 data even with d = 4 or 5. But the adf test did, so let's try D = 4 for the 2020 data.
...
To check seasonality, let's extract the dominant frequency from each time series object.

```{r}
#Extract frequency from 2019 data.
forecast::findfrequency(tsdf2019mets_diff3)
#Extract frequency from 2020 data.
forecast::findfrequency(tsdf2020mets_diff4)
```
...
ts2019_stat <-  ts(tsdf2019mets_diff3[1:8], frequency = 3)
#Fit the apropriate time series for 2020 data.
ts2020_stat <- ts(tsdf2020mets_diff4[1:8], frequency = 3)
```
Now that we have a stationary time series for each year, we should select the appropriate ARIMA model. This means finding the most appropriate values for P, p, Q and q in the ARIMA(p,d,q)X(P,D,Q) model, as we already solved for D.

As a quick refresher, p refers to how many previous (lagged) *Y* values are accounted for at each point, and q refers to how many previous (lagged) *error* values are accounted for each time point in our model. We'll use two tools here:
1. Correlogram
1. Partial correlogram
...
acf(as.numeric(ts2019_stat))
pacf(as.numeric(ts2019_stat))

acf(as.numeric(ts2020_stat))
pacf(as.numeric(ts2020_stat))

```

The largest statistically significant lag values of the correlogram gives the possible seasonal *q* values for the ARIMA model. In addition, the largest statistically significant lag of the partial correlogram gives the seasonal *p* value for an ARIMA model. It appears we want P,Q = 0,0 for 2019 and 2020.

Lets grab the nonseasonal components of the data by seasonal decomposition.

```{r fig.height= 6, fig.width = 6}
#2019
seasadj2019 <- forecast::seasadj(stl(ts2019_stat, s.window = "periodic"))
acf(as.numeric(seasadj2019))
pacf(as.numeric(seasadj2019))

#2020
seasadj2020 <- forecast::seasadj(stl(ts2020_stat, s.window = "periodic"))
acf(as.numeric(seasadj2020))
pacf(as.numeric(seasadj2020))
```

So we're left with
1. 2019: (p,d,q)x(p,D,Q) = (0,0,0)(0,3,0)
1. 2020: (p,d,q)x(p,D,Q) = (1,0,2)(0,4,0)

Let's fit the models.

```{r}
#Fit both ARIMA models.
arima2019 <- forecast::Arima(tsdf2019mets, order =  c(0,0,0), seasonal = c(0, 3, 0))
arima2020 <- forecast::Arima(tsdf2020mets, order =  c(1,0,2), seasonal = c(0,4,0))
```
