...
```{r}
#Fit the regression.
fit <- lm(mets ~ weeks + year+ weeks*year, data  = df)
```
...
#2019
df2019 <- df[1:11,]
ggplot(data = df2019, mapping = aes(weeks, mets)) +
  geom_point() +
  geom_abline(intercept = coefficients(fit)[1],
              slope = coefficients(fit)[2])
#2020
df2020 <- df[12:22,]
ggplot(data = df2020, mapping = aes(weeks, mets)) +
  geom_point() +
  geom_abline(intercept = coefficients(fit)[1],
              slope = coefficients(fit)[2])
summary(fit)
...
df$year <- c(rep(0, 11), rep(1, 11)) 
#Fit the regression.
fitvol <- lm(vol ~ weeks + year + weeks*year, data  = df)
```
...
#2019
df2019 <- df[1:11,]
ggplot(data = df2019, mapping = aes(weeks, vol)) +
  geom_point() +
  geom_abline(intercept = coefficients(fitvol)[1],
              slope = coefficients(fitvol)[2])
#2020
df2020 <- df[12:22,]
ggplot(data = df2020, mapping = aes(weeks, vol)) +
  geom_point() +
  geom_abline(intercept = coefficients(fitvol)[1],
              slope = coefficients(fitvol)[2])
summary(fitvol)
