...
## Comparing 2019 vs 2020 met minute time trends
...
Checking and fitting ARIMA (0,0,0):
...
*Conclusions*
Since, our data is white noise we're not going to be able to forecast using classical time-series models This makes our original plan for intervention analyses unachievable unless we switch models.

###Linear Regression
What we can do is test whether a linear time trend differs between 2019 and 2020.
```{r}
df <- RMHF::read_data()
...
*Methodology*
What we're going to do here is include an interaction term between year (2019, 2020) and time, whose coefficient (trend) we want to compare. Since we think that time might have a different effect for 2019 and 2020, we include an interaction term between year and each time(weeks). Then, we fit  a regression model to the data. The coefficient for weeks will is the coefficient for weeks for the reference group (2019) only. The interaction term between year and weeks represents the difference in the coefficients between 2019 and 2020. Lastly, to obtain the coefficient for 2020 (the comparison group), all we need to to is add the coefficients for the predictor (weeks) on its own and week*year interaction. This will give us a p value, which is good. So let's fit the regression line. 
...
Let's check the assumptions of our model using a few key diagnostic plot.
...
#2019
df2019 <- df[1:11,]
ggplot(data = df2019, mapping = aes(weeks, mets)) + geom_point() + geom_abline(intercept = coefficients(fit)[1],
                                                                           slope = coefficients(fit)[2])
#2020
df2020 <- df[12:22,]
ggplot(data = df2020, mapping = aes(weeks, mets)) + geom_point() + geom_abline(intercept = coefficients(fit)[1],
                                                                           slope = coefficients(fit)[2])
...
### MET minutes
If we fit an ARIMA model to the 2020 [Jan 1 - March11] data automatically, we yield the following automatic model fit. We'll check diagnostic plots for this model too, to check the validity of our assumptions.
...
*The good news:*
All the autocorrelations for the residual series are non-significant, which is what we'd like. The model passes the Box-Ljung test as well. In other words, our model appears to work.

*The bad news:*
Since our model is a white noise model, it is not forcastable by definition because its values at different times are statistically insignificant. From the best fit of our data being a white noise model, we know that no ARMA model can make prediction better than a long run mean.

*A solution*
_Remember, we'd like to model the intervention effect. To do so though, we need good forecasts of values after the intervention using those before. So let's try a model that can involve nonlinear relationships between our predictors and MET-minutes - an Artificial Neural Network. Neural networks are a form of deep learning that are loosely based on the arrangement of neurons in the human nervous system. We're going to be using recurrent neural networks, which are good for robustness to noise, automatic feature extraction, and modelling more complex relationships in the data. They also take multivariate inputs and produce multivariate outputs, which is a feature we'd like our model to have. Our time series intervention question will therefore start with a prediction question, using supervised machine learning.

Given a sequence of numbers for the met-minutes time series, we need to restructure the data to look like a supervised learning problem. We can do this by using previous time steps as input variables and use the next time step as the output variable. This is called a sliding window (we'll use a width of 2) transformation. This is because it functions similarily to sliding a window across prior observations that are used as inputs to the model in order to predict the next value in the series.

 So we'll take pre March 16 MET-minutes, and turn it into the following.

```{r}
x <- data.frame("1" = RMHF::read_data()[c(12, 13, 14, 15), "Met-minutes"],
                      "2" = RMHF::read_data()[c(13, 14, 15, 16), "Met-minutes"])
y <- data.frame(RMHF::read_data()[c(14, 15, 16, 17), "Met-minutes"])
```

Now, we'll use TensorFlow to fit a neural network to the data.
