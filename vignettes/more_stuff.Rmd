...
```
Now we can test for the significance of the difference before to after the clinic closed on the 13th of March, after adjusting for 2019 data.

```{r}
library(rstatix)
anova_test(data = df, formula = posttest ~ group)
```
...
df <- data.frame("pretest" = covariate, "posttest" = var2020, "group" = group, stringsAsFactors = FALSE)
```
Now we can test for the significance of the difference before and after the clinic closed on the 13th of March, after adjusting for 2019 data.
```{r}
anova_test(data = df, formula = posttest ~ pretest + group)
```
...
In conclusion, after adjusting for 2019 data, the intervention had very little significant effect on 2020 data for any of the variables studied after adjusting for 2019 data.
